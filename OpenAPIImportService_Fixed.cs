    ...
    public class OpenAPIImportService : IOpenAPIImportService
    {
        ...
        public async Task<OpenAPIImportResponse> ImportFromFileContentAsync(string fileContent, string? baseUrl = null, List<string>? selectedOperations = null)
        {
            try
            {
                ...
                _logger.LogInformation("Selected operations count: {Count}", selectedOperations?.Count ?? 0);
                ...
                _logger.LogInformation("Created integration with {RequestCount} requests", importedCount);
                ...
                foreach (var path in document.Paths)
                {
                    foreach (var operation in path.Value.Operations)
                    {
                        if (operation.Value != null)
                        {
                            ...
                            _logger.LogInformation("Added request: {RequestName} ({Method} {Url})", request.Name, request.Method, request.Url);
                        }
                    }
                }

                integration.Requests = requests;
                _logger.LogInformation("Integration created with {RequestCount} total requests", requests.Count);
                return integration;
            }
            catch (Exception ex)
            {
